
/////////////////////////////////////////////////////////////////////////////////
const answers1 = {
  answer1: 'correct',
  answer2: 'wrong',
  answer3: 'wrong',
};
const str1 = JSON.stringify(answers1);
console.log(str1);

const answers2 = {
  answer1: 'wrong',
  answer2: 'correct',
  answer3: 'wrong',
};
const str2 = JSON.stringify(answers2);
console.log(str2);

const answers3 = {
  answer1: 'wrong',
  answer2: 'wrong',
  answer3: 'correct',
};
const str3 = JSON.stringify(answers2);
console.log(str2);

/////////////////////////////////////////////////////////////////////////////////
$(function() {
   let html = $('#header').html();

   let header = [
      {
        title: 'Тест по программированию'
      },
   ];

  let content = tmpl(html, {
    data: header
  });
   $('header').append(content);
});

////////////////////////////////////////////////////////////////////////////////

$(function() {
   let html = $('#question1').html();

   let question1 = [
      {
        title: 'Вопрос №1',
        content: 'Вариант ответа №'
      },
   ];

  let content = tmpl(html, {
    data: question1,
  });
   $('body .question1').append(content);
});

//////////////////////////////////////////////////////////////////////////////
$(function() {
   let html = $('#question2').html();

   let question2 = [
      {
        title: 'Вопрос №2',
        content: 'Вариант ответа №'
      },
   ];

  let content = tmpl(html, {
    data: question2,
  });
   $('body .question2').append(content);
});

////////////////////////////////////////////////////////////////////////////////
$(function() {
   let html = $('#question3').html();

   let question3 = [
      {
        title: 'Вопрос №3',
        content: 'Вариант ответа №'
      },
   ];

  let content = tmpl(html, {
    data: question3,
  });
   $('body .question3').append(content);
});

///////////////////////////////////////////////////////////////////////////////
$(function() {
   let html = $('#check').html();

   let check = [
      {
        content: 'Проверить свои результаты'
      },
   ];

  let content = tmpl(html, {
    data: check
  });
   $('body .check').append(content);
});

/////////////////////////////////////////////////////////////////////////////////
    $ (function() {

       let $test = $('button')
       $test.on('click', function () {
         let counter = 0;
            if (q1.answer1[0].checked) counter++;
            if (q2.answer2[1].checked) counter++;
            if (q3.answer3[2].checked) counter++;

             $("input").prop("checked", false);
          let result;
            if (counter == "0") result = "ВСЕ ПЛОХО! УЧИТЕ ЕЩЕ";
            if (counter == "1") result = "УДОВЛЕТВОРИТЕЛЬНО";
            if (counter == "2") result = "ХОРОШО";
            if (counter == "3") result = "ВЕЛИКОЛЕПНО";

         alert(counter + " правильных ответов" + ". " + "Ваша оценка" + ": " + result)
       });
 });
